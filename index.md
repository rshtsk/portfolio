...
### Проект: Анализ экономики продукта и маркетинговых метрик
[Посмотреть на GitHub →](https://github.com/rshtsk/sql_product_analytics)

**Описание:**  
Комплексный SQL-проект, охватывающий анализ выручки, затрат, прибыльности, удержания и эффективности рекламных кампаний. Выполнен на учебном датасете.  
Реализован полный цикл продуктовой аналитики — от расчёта метрик до визуализации результатов.

**Основные метрики:**
- ARPU / ARPPU / AOV (включая накопительные)
- Выручка и валовая прибыль
- Retention (когортный и по рекламным кампаниям)
- CAC / ROI<br><br>
**Стек:** `SQL` (PostgreSQL), `Redash` <br><br>
**Навыки и темы:**<br>
- Работа с оконными функциями (`window functions`)
- Расчёт когорт и retention
- Финансовые метрики (выручка, налоги, затраты)
- Декомпозиция выручки по пользователям и товарам
...
**Что сделала:**
1. Спроектировала архитектуру внутренней CRM-системы для автоматизации бизнес-процессов туристического клуба:
- Проанализировала процессы работы с участниками, турами, оплатами, договорами и отзывами  
- Разработала реляционную модель данных (PostgreSQL) с учётом связей, ограничений, ролей и комментариев  
- Подготовила ER-диаграмму, описания всех сущностей и их полей для разработчиков и заказчика  
- Составила user flow для сотрудников: добавление участников, регистрация на туры, генерация договоров, учёт оплат  
- Продумала логику автоматизации: генерация договоров, интеграция с Telegram-ботом и платёжной системой  
- Учла требования к масштабированию, роли пользователей, подключению внешних интерфейсов и аналитических дашбордов

**Стек:** PostgreSQL, Telegram Bot (aiogram), docxtpl, FastAPI (планируемый), Redash / Yandex DataLens  
**Результат:** Проектная документация, согласованная структура БД, макеты интерфейсов и сценариев
...
- Определила ключевые метрики для облегчения принятий бизнес-решений:  
